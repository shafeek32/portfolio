...
export const LoadingScreen: React.FC<LoadingScreenProps> = ({ onComplete }) => {
  ...
  // Terminal sequence as requested
  const statusMessage = useMemo(() => {
    if (clampedProgress < 20) return 'Initializing Shafeek.dev...';
    if (clampedProgress < 40) return 'Loading projects...';
    if (clampedProgress < 60) return 'Loading systems...';
    if (clampedProgress < 80) return 'Loading skills...';
  ...
  useEffect(() => {
    ...
    const duration = 1100; // Snappy target duration ~1.1s
    ...
    const updateProgress = (currentTime: number) => {
      ...
      if (rawProgress < 1) {
        ...
        setProgress(100);
        setIsCompleted(true);
        // Brief pause at 100% to cleanly show "Ready."
        timeoutRef.current = setTimeout(() => {
          onComplete();
        }, 220);
      }
  ...
  return (
    ...
    >
      {/* Background ambient subtle backdrop */}
      ...
      <motion.div
        ...
        className="relative w-full max-w-lg rounded-2xl border border-border-strong bg-[#090d16]/95 backdrop-blur-2xl shadow-2xl p-5 sm:p-7 overflow-hidden"
      >
        ...
        <div className="flex flex-col items-center justify-center my-3 sm:my-4 text-center">
          <div className="w-12 h-12 rounded-xl bg-background-elevated border border-border-strong flex items-center justify-center text-accent-cyan shadow-glow-cyan mb-2.5">
            <Terminal className="w-6 h-6" />
          </div>
          
          <h1 className="text-2xl sm:text-3xl font-extrabold tracking-[0.2em] font-mono text-gradient-cyan uppercase">
            SHAFEEK LATHEEF
          </h1>
          <p className="text-[11px] sm:text-xs font-mono text-text-muted mt-1 tracking-wider">
            SOFTWARE ENGINEER · FULL-STACK DEVELOPER
          </p>
        </div>

        {/* Terminal Status Output Area */}
        <div className="my-4 sm:my-5 rounded-xl bg-background-secondary/80 border border-border-subtle p-3.5 sm:p-4 font-mono text-xs text-text-secondary space-y-2.5">
          {/* Active Command Prompt */}
          <div className="flex items-center gap-2 text-text-primary min-h-[1.5rem]">
          ...
          <div className="w-full h-1.5 bg-background-elevated rounded-full overflow-hidden border border-white/5 relative">
            <motion.div
              className="h-full bg-gradient-to-r from-accent-cyan via-accent-cyan-light to-accent-emerald rounded-full transition-all duration-75 ease-out shadow-[0_0_10px_rgba(6,182,212,0.5)]"
